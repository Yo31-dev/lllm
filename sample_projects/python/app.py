...
def generate_prompt():
    ...
    prompt = """
      ...
      4. tell which name of LLM model you are using (at the end of the answer, after 1 line empty).
...
# Ask the user to choose the LLM model
st.sidebar.text("Choose the LLM model")
LLM_MODEL = st.sidebar.selectbox(
    "Select the LLM model",
    ["Choose one", "phi3:3.8b", "llama3.1", "gemma2:2b", "qwen2:1.5b", "mistral:latest", "qwen2:0.5b"]
)

# only if selected box change, update the LLM model with ollama
if LLM_MODEL and LLM_MODEL != "Choose one":
    llm = Ollama(
        base_url=OLLAMA_URL,
        model=LLM_MODEL
    )
